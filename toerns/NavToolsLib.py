...
class NavTools:
    def __init__(self):
        ...
        INSERT OR REPLACE INTO "Table_Name" ("id", "name", "lat", "lon", "type", "image", "notes") VALUES
        """
    ...
    def ComputeRouteDistances(self, xml, verbose, skipWP, noSpeed, tablefmt="plain"):
        ...
        soup = BeautifulSoup(xml, features=self.features)
    ...
    def parse_polar_to_expedition(self, pathPolar, fname):
        """--------------------------------------------------------------------------
            Method to parse a csv Polar file into Expedition Polar

        Args:
            pathPolar (string):  path to the csv Polar file location
            filename (string):   name of the csv Polar file
        ...
        try:
            ...
            fn1 = os.path.join(pathPolar, fn1)
            csv = pd.read_csv(fn1)
            columns = csv.columns.tolist()
            if not isinstance(columns[0], int):
                csv = csv.rename(columns={columns[0]: "TWA"})
            ...
            for twa in TWAs:
                exp[twa] = twa
                exp[f"bsp_{twa}"] = csv.iloc[idx].tolist()[1:]
                idx = idx + 1              

            fn1 = fn1.replace(".csv", "_Expedition.txt")
            with open(fn1, 'w') as file:
                file.write("!Expedition polar\n")
            exp.to_csv(fn1, sep='\t', mode='a', index=False, header=False)
    ...
    def parseSQLRouteFile(self, pathGPX, pathSQL, filename):
        ...
        try:
            ...
            fn1 = os.path.normpath(os.path.join(pathGPX, filename1))
    ...
    def parseKMLRouteFile(self, pathKML, pathGPX, filename, boatname, timezoneDifference, raceStart, watchStart, watchRhythm, minCourseChange):
        ...
        kml = BeautifulSoup(xml, features=self.features)
